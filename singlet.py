...
class model1(generic_potential.generic_potential):
    ...
    """
    def init(self,mu2=-8720.10,mus2=-4860.43,lamda=0.129,lamda2=0.1,lamda3=0.3,v=246,vs=0,yt=0.9946,g=0.6535,g1=0.35):
    ...
    def boson_massSq(self, X, T):
        ...
        b = 2*1000 + 12* self.lamda2 * s**2 + self.lamda3 * h**2
        c0 = 4*self.lamda32 * h**2 * s**2
        A = 0.5*(a+b)
        B = 0.5*( np.sqrt((a-b)**2 + 4*c0))
        W2=(self.g*h/2)**2
        Z2=(h*np.sqrt(self.g**2+self.g1**2)/2)**2
    ...
    def fermion_massSq(self, X):
        ...
        m12 = (self.yt*h/np.sqrt(2))**2 # First fermion mass
        massSq = np.array([m12])
    ...
    def approxZeroTMin(self):
        ...
        return [np.array([0,0]), np.array([self.v,self.vs])]
...
def makePlots(m=None):
    import matplotlib.pyplot as plt
    if m is None:
        #m=model1(mu2,mus2,lamda,lamda2,lamda3,v,vs,yt,g,g1)
        m = model1(-8720.10,-4860.43,0.129,0.1,0.3,246,0,0.9946,0.6535,0.35)
