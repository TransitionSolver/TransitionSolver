...
class Phase:
    ...
    def find_phase_at_t(self, T: float, potential) -> np.ndarray:
        if T < self.T[0] or T > self.T[-1]:
            raise RuntimeError(
                f"Attempted to find phase {self.key} at T={T}, while defined only for"
                f"[{self.T[0]}, {self.T[-1]}]"
            )
        ...
        def V(X) -> Union[float, np.ndarray]:
        ...
        if minIndex == maxIndex:
            ...
            interpFactor = (T - self.T[minIndex]) / (
                self.T[maxIndex] - self.T[minIndex]
            )
            interpPoint = self.phi[..., minIndex] + interpFactor * (
                self.phi[..., maxIndex] - self.phi[..., minIndex]
            )

        direction = np.diag(np.ones(potential.get_n_scalars()) * offset)
        optimisedPoint = optimize.fmin_powell(
            V, interpPoint, disp=False, direc=direction
        )
        ...
        if (
            abs(T - self.T[minIndex]) < 0.2 * potential.get_temperature_scale()
            and np.linalg.norm(optimisedPoint - interpPoint)
            > 0.2 * potential.get_field_scale()
        ):
...
class TransitionProperties(dict):
    ...
    @property
    def completed(self):
        return self.T_f is not None and self.T_f >= 0.0
...
class Transition:
    ...
    def __init__(self, transition: np.ndarray):
        ...
        # use integer for numeric sorting, stringify when outputting json reports
        # to avoid ambiguity in dict keys when serialized
        ...
        self.properties.subcritical = bool(
            transition[-1] > 0
