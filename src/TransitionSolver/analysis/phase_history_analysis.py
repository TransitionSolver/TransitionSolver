...
class PhaseHistoryAnalyser:
    ...
    def report(self):
        ...
        report["transitions"] = {
            str(t.ID): t.report() for t in self.phase_structure.transitions
        }
        return report
    ...
    def is_high_temperature_phase(self):
        ...
        return [temp == max_temperature for temp in highest_temperatures]
    ...
    def phase_indexed_trans(self, phase_nodes):
        ...
        phase_indexed_trans: list[list[TransitionEdge]] = [
            [] for _ in range(len(self.phase_structure.phases))
        ]
        ...
        for i, ut in enumerate(self.unique_transition_temperatures):
            ...
            for j in range(self.unique_transition_idx[i], max_idx):
                ...
                false_phase_node_index = next(
                    (
                        k
                        for k, node in enumerate(phase_nodes[false_phase])
                        if node.temperature == ut
                    ),
                    None,
                )
                true_phase_node_index = next(
                    (
                        k
                        for k, node in enumerate(phase_nodes[true_phase])
                        if node.temperature == ut
                    ),
                    None,
                )
                ...
                if false_phase_node_index is None or true_phase_node_index is None:
                    raise KeyError(
                        f"Missing PhaseNode at T_c={ut} for false={false_phase}, true={true_phase}"
                    )

                transition_edge = TransitionEdge(
                    phase_nodes[false_phase][false_phase_node_index],
                    phase_nodes[true_phase][true_phase_node_index],
                    self.phase_structure.transitions[j],
                    len(phase_indexed_trans[false_phase]),
                )
    ...
    def phase_nodes(self):
        ...
        for i, p in enumerate(self.phase_structure.phases):
            ...
            nodes_i = [
                PhaseNode(i, t)
                for t in self.unique_transition_temperatures
                if Tmax >= t >= Tmin
            ]
    ...
    def trivial_paths(self):
        ...
        for i, phase in enumerate(self.phase_structure.phases):
            if not (
                self.is_high_temperature_phase[i] and self.is_low_temperature_phase[i]
            ):
    ...
    def analyse_transition(
        self, phase_indexed_trans, transition_edge, path, transition, **kwargs
    ):
        ...
        if Tmin < Tmax:
            ...
            ta = TransitionAnalyser(
                self.potential,
                transition.properties,
                self.phase_structure.phases[transition.false_phase],
                self.phase_structure.phases[transition.true_phase],
                self.phase_structure.ground_state_energy_density,
                Tmin=Tmin,
                Tmax=Tmax,
                **kwargs,
            )
    ...
    def analyse(self, bubble_wall_velocity=None, action_ct=False):
        ...
        while frontier:
            ...
            self.analyse_transition(
                phase_indexed_trans,
                transition_edge,
                path,
                transition,
                bubble_wall_velocity=bubble_wall_velocity,
                action_ct=action_ct,
            )

            # If the transition completes.
            if (
                transition.properties.T_p is not None
                and transition.properties.T_f is not None
            ):
                # Guard against NaN silently passing through.
                if math.isnan(transition.properties.T_p) or math.isnan(
                    transition.properties.T_f
                ):
                    raise ValueError(
                        f"Invalid milestone temperature(s): T_p={transition.properties.Tp},"
                        f"T_f={transition.properties.Tf} for transition ID={transition.ID}"
                    )

                Tf = transition.properties.T_f
                # discard histories involving transitions out of the phase which is the false vacuum
                # in the transition that completed
                ...
                frontier = [
                    ...
                    for f in frontier
                    if not (
                        f.false_phase_node.phase
                        == transition_edge.false_phase_node.phase
                ...
                if not true_phase.paths:
                    ...
                    frontier += self.new_frontier(
                        transition_edge, phase_indexed_trans, path, True
                    )
                    frontier.sort(
                        key=lambda tr: tr.transition.properties.T_c, reverse=True
                    )
            ...
            # If the transition doesn't begin.
                ...
                # critical temperature).
                frontier += self.new_frontier(
                    transition_edge, phase_indexed_trans, path, False
                )
                frontier.sort(key=lambda tr: tr.transition.properties.T_c, reverse=True)
    ...
    def set_is_valid(self):
        for p in self.paths:
            p.is_valid = (
                not p.suffix_links and self.is_low_temperature_phase[p.phases[-1].phase]
            )
    ...
    def new_frontier(
        ...
        if transition_edge.index < len(phase_indexed_trans[false_phase]) - 1:
            Tc = phase_indexed_trans[false_phase][
                transition_edge.index + 1
            ].transition.properties.T_c
            ...
            for transition in path.transitions:
                if (
                    transition.properties.T_c
                    > transition_edge.transition.properties.T_c
                    and transition.properties.T_f > Tc
                ):
            ...
            already_transitioned = (
                already_transitioned
                or transition_edge.transition.properties.T_f is not None
            )

            if not already_transitioned:
                new_frontier.append(
                    phase_indexed_trans[false_phase][transition_edge.index + 1]
                )
        ...
        if transition_starts:
            ...
            for i in range(len(phase_indexed_trans[true_phase])):
                ...
                if (
                    self.phase_structure.phases[new_transition.true_phase].T[0]
                    > transition_edge.transition.properties.T_n
                ):
                    continue
    ...
    def max_trans_temperature(self, path, transition):
        if path.transitions:
            ...
            for tr in reversed(path.transitions):
                if (
                    tr.true_phase == transition.false_phase
                    and tr.properties.T_n is not None
                ):
                    return min(transition.properties.T_c, tr.properties.T_n)
        return transition.properties.T_c

    def min_trans_temperature_idxed(
        self,
        phase_indexed_trans: list[list[TransitionEdge]],
        transition_edge: TransitionEdge,
    ) -> float:
        ...
        for p in phase_indexed_trans[transition_edge.transition.true_phase][
            transition_edge.index + 1:
        ]:
            reverse_transition = p.transition
            if (
                reverse_transition.properties.T_c
                < transition_edge.transition.properties.T_c
                and reverse_transition.true_phase
                == transition_edge.transition.false_phase
            ):
        ...
        return max(
            Tmin,
            self.phase_structure.phases[transition_edge.transition.false_phase].T.min(),
            self.phase_structure.phases[transition_edge.transition.true_phase].T.min(),
        )
