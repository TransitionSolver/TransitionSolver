...
from . import build_phase_tracer, run_phase_tracer, make_phase_history
...
def cli(model, model_header, model_lib, model_namespace, point_file_name, vw, detector, pta, show, level, apply, force):
    ...
    with Status(f"Running PhaseTracer {exe_name}"):
        phase_structure = run_phase_tracer(exe_name, point_file_name)
    ...
    with Status("Analyzing gravitational wave signal"):
        analyser = gws.GWAnalyser(potential, phase_structure, phase_history, vw=vw)
