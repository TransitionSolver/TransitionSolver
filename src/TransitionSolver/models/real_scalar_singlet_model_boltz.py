...
class RealScalarSingletModel_Boltz(AnalysablePotential):
    def init(self, c1, b3, theta, vs, ms, muh=0, mus=0, lh=0, ls=0, c2=0, muh0=0, mus0=0, bDebugIteration=False,
            bStoreParameterEvolution=False, bSkipOneLoop=False, bUseBoltzmannSuppression=True):
    ...
    def calculateMixingAngle_tree_tan(self, X):
        mhh = self.d2V0mdh2(X)
    ...
    def massEigenvalues(self, X, T=0, massMatrix=None):
        if massMatrix is None:
            mhh0 = self.d2V0mdh2(X)
            mss0 = self.d2V0mds2(X)
            ...
            if self.bUseBoltzmannSuppression:
                Th = Th*self.getBoltzmannSuppression(mhh0, TSq)
                Ts = Ts*self.getBoltzmannSuppression(mss0, TSq)
    ...
    def massEigenvalues_full(self, X, T=0, massMatrix=None):
        if massMatrix is None:
            ...
            #if self.bUseBoltzmannSuppression:
            #    Th = 0. if TSq == 0. or mhh0/TSq > BOLTZ_EXP_MAX else Th*np.exp(-mhh0/TSq)
            #    Ts = 0. if TSq == 0. or mss0/TSq > BOLTZ_EXP_MAX else Ts*np.exp(-mss0/TSq)

            mhh = mhh0 + Th*self.getBoltzmannSuppression(mhh0, TSq)
            mhs = self.d2Vdhds(X)
            mss = mss0 + Ts*self.getBoltzmannSuppression(mss0, TSq)
    ...
    def goldstoneMass(self, X, T=0, massMatrix=None):
        ...
        if self.bUseGoldstoneResummation:
            ...
            mass = 1/h * self.dV0mdh(xCopy)

        if T > 0:
            #TSq = T*T
            #thermalCorrections = np.zeros(shape=mass.shape)
            #mask = mass/TSq < BOLTZ_EXP_MAX
            #thermalCorrections[mask] = self.debyeCorrectionHiggs(T)*np.exp(-np.abs(mass[mask])/TSq)
            thermalCorrections = self.debyeCorrectionHiggs(T)*self.getBoltzmannSuppression(mass, T * T)
            mass += thermalCorrections
    ...
    def boson_massSq(self, X, T, massMatrix=None, ignoreGoldstone=False):
        ...
        mZSq_T = (g2Sq + g1Sq)/4*hSq

        if self.bUseBoltzmannSuppression and TSq > 0:
            #a0 = b0 = (g2Sq + g1Sq)*3*hSq
            #mZSq_T = (a0 + b0)/24
            #ph2_T = (a0 - b0)/24
            #TSqForZ = (0. if mZSq_T/TSq > BOLTZ_EXP_MAX else TSq*np.exp(-mZSq_T/TSq))\
            #    if self.bUseBoltzmannSuppression else TSq
            TSqForZ = TSq*self.getBoltzmannSuppression(mZSq_T, TSq)
            a = (g2Sq + g1Sq)*(3*hSq + 22*TSqForZ)
            b = np.sqrt(9*(g2Sq + g1Sq)**2*hSq**2 + 44*TSqForZ*(g2Sq - g1Sq)**2 * (3*hSq + 11*TSqForZ))
        else:
            a = (g2Sq + g1Sq)*(3*hSq + 22*TSq)
            b = np.sqrt(9*(g2Sq + g1Sq)**2*hSq**2 + 44*TSq*(g2Sq - g1Sq)**2*(3*hSq + 11*TSq))
        ...
        mWSq_T = g2Sq*hSq/4
        mWSq_L = mWSq_T + 11/6*self.g2*TSq*self.getBoltzmannSuppression(mWSq_T, TSq)
        ...
        massSq = np.empty(mZSq_L.shape + (Nboson,))
        ...
        dof = np.array([3, 2, 4, 1, 2, 1, *[1]*self.Ndim])
        c = np.array([1.5, 5/6, 5/6, 5/6, 5/6, 5/6, *[1.5]*self.Ndim])
    ...
    def fermion_massSq(self, X):
        ...
        #mt = self.yt2*h2/2
        # From PDG 2023.
        topSq = 162.5**2*hSq/vhSq
        upSq = 0.00216**2*hSq/vhSq
        downSq = 0.00467**2*hSq/vhSq
        strangeSq = 0.0934**2*hSq/vhSq
        charmSq = 1.27**2*hSq/vhSq
        bottomSq = 4.18**2*hSq/vhSq
        muonSq = 0.10566**2*hSq/vhSq
        tauonSq = 1.777**2*hSq/vhSq

        massSq = np.empty(np.asarray(topSq).shape + (Nfermions,))
    ...
    def d_fermion_massSq_dh(self, X):
        ...
        # Top quark.
        #mt = self.yt2*h
        topSq = 2*162.5**2*h/vhSq
        upSq = 2*0.00216**2*h/vhSq
        downSq = 2*0.00467**2*h/vhSq
        strangeSq = 2*0.0934**2*h/vhSq
        charmSq = 2*1.27**2*h/vhSq
        bottomSq = 2*4.18**2*h/vhSq
        muonSq = 2*0.10566**2*h/vhSq
        tauonSq = 2*1.777**2*h/vhSq
    ...
    def d_fermion_massSq_ds(self, X):
        ...
        massSq = np.zeros(X[..., 0].shape + (Nfermions,))
    ...
    def d2_fermion_massSq_dh2(self, X):
        ...
        # Top quark.
        #mt = self.yt2
        topSq = 2*162.5**2/vhSq
        upSq = 2*0.00216**2/vhSq
        downSq = 2*0.00467**2/vhSq
        strangeSq = 2*0.0934**2/vhSq
        charmSq = 2*1.27**2/vhSq
        bottomSq = 2*4.18**2/vhSq
        muonSq = 2*0.10566**2/vhSq
        tauonSq = 2*1.777**2/vhSq
    ...
    def constrainParameters(self, bDebugIteration=False, bStoreParameterEvolution=False, bSkipTreeLevel=False, bSkipOneLoop=False):
        ...
        self.thetaPrev = self.calculateMixingAngle_tree_tan(vev)
...
if __name__ == "__main__":
    ...
    _pot = RealScalarSingletModel_Boltz(*_params)
