...
def makePfPlot():
    ...
    line = plt.plot(T1, Pf1, linewidth=2.5, zorder=(5 if T1[0] > T2[0] else 3), c='b')
    legendHandles.append(line)
    line = plt.plot(T2, Pf2, linewidth=2.5, zorder=(5 if T2[0] > T1[0] else 3), c='r')
    legendHandles.append(line)
    if Tn1 > 0: plt.axvline(Tn1, ymax=1., lw=2, c='b', ls=':', label='$T_n$')
    if Tn2 > 0: plt.axvline(Tn2, ymax=1, lw=2, c='r', ls=':', label='_nolegend_')
    if Tp1 > 0: plt.axvline(Tp1, ymax=0.71, lw=2, c='b', ls='--', label='$T_p$')
    if Tp2 > 0: plt.axvline(Tp2, ymax=0.71, lw=2, c='r', ls='--', label='_nolegend_')
    if Tf1 > 0: plt.axvline(Tf1, ymax=0.01, lw=2, c='b', ls='-.', label='$T_f$')
    if Tf2 > 0: plt.axvline(Tf2, ymax=0.01, lw=2, c='r', ls='-.', label='_nolegend_')
