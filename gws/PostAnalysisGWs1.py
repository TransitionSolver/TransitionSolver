...
class GWAnalyser_InidividualTransition:
    ...
    def determineGWs(self) -> tuple[float, float]:
        ...
        self.peakAmplitude = 0.15509*redshift * K*K * H*(lenScale_primary/(8*np.pi)**(1./3.))/soundSpeed * upsilon #计算了除了spetral shape 的前面几项
        self.peakAmplitudeturb = 3.35 * 10e-4 * K**(3/2) * H*(lenScale_primary/(8*np.pi)**(1./3.))/soundSpeed    #计算turbulence
    ...
    def spectralShape(self, f: float) -> float:         #spectralShape 1
    ...
    def spectralShape_doubleBroken(self, f: float):     #spectralShape 2 turblence
    ...
    def __init__(self, detectorClass: Type[GWDetector], potentialClass: Type[AnalysablePotential], outputFolder: str):
        ...
        for transitionReport in self.relevantTransitions:
            gws = GWAnalyser_InidividualTransition(self.phaseStructure, transitionReport, self.potential,
                self.detector)
            gwFunc_single = gws.getGWfuncturb(doubleBroken=False)
            gwFunc_double = gws.getGWfuncturb(doubleBroken=True)
            ...
            print('Peak amplitude:', gws.peakAmplitudeturb)
            print('Peak frequency (primary):', gws.peakFrequency_primaryturb)
            print('Peak frequency (secondary):', gws.peakFrequency_secondaryturb)
