...
class GWAnalyser_InidividualTransition:
    ...
    def determineGWs(self) -> tuple[float, float]:
        ...
        if K == 0:
            return 0., 0.
        lenScale_primary = self.determineLengthScale() #mean bubble seperation
        ...
        upsilon = 1 - 1 / np.sqrt(1 + 2*H*tau_sw)  #计算得到了r(taosw)
        upsilon_old = min(1,H*tau_sw)
        print("lenScale_primary = ",lenScale_primary)
        print("adiabaticIndex = ",adiabaticIndex)
        print("K = ",K)
        print("tau_sw = ",tau_sw)
        print("H = ",H)
        print("H*tau_sw =",H*tau_sw)

        soundSpeed = np.sqrt(self.hydroVars.soundSpeedSqFalse)
        #tau_c = lenScale_primary / soundSpeed
        lenScale_secondary = lenScale_primary * abs(vw - np.sqrt(self.hydroVars.soundSpeedSqFalse)) / vw  #(secondary)
        ...
        self.peakAmplitudesw = 0.15509*redshift * K*K * H*(lenScale_primary/(8*np.pi)**(1./3.))/soundSpeed * upsilon #计算了除了spetral shape 的前面几项
        
        
        #self.peakAmplitudeturb = 3.35 * 10e-4 * (0.1*K)**(3/2) * H*(lenScale_primary/(8*np.pi)**(1./3.))/soundSpeed    #计算turbulence
        #self.peakAmplitudeturb = 3.35 * 10e-4 * (0.05*K)**(3/2) * H*(lenScale_primary/(8*np.pi)**(1./3.))/soundSpeed
        self.peakAmplitudeturb = 3.35 * 10e-4 * ((1 - upsilon)**(2/3)*K)**(3/2) * H*(lenScale_primary/(8*np.pi)**(1./3.))/soundSpeed
        #self.peakAmplitudeturb = 3.35 * 10e-4 * ((1 - upsilon_old)**(2/3)*K)**(3/2) * H*(lenScale_primary/(8*np.pi)**(1./3.))/soundSpeed
        ...
        self.peakFrequency_primarysw = 8.9e-6*(self.potential.ndof/100)**(1./6.)*(Treh/100)/(H*lenScale_primary/(8*np.pi)**(1./3.))*(zp/10) 
        self.peakFrequency_secondarysw = 8.9e-6*(self.potential.ndof/100)**(1./6.)*(Treh/100)/(H*lenScale_secondary/(8*np.pi)**(1./3.))*(zp/10)
            
            
            
        self.peakFrequency_primaryturb = 2.7e-6*(self.potential.ndof/100)**(1./6.)*(Treh/100)/(H*lenScale_primary/(8*np.pi)**(1./3.))
        self.peakFrequency_secondaryturb = 2.7e-6*(self.potential.ndof/100)**(1./6.)*(Treh/100)/(H*lenScale_secondary/(8*np.pi)**(1./3.))
    ...
    def spectralShapesw(self, f: float) -> float:         #spectralShape 1
        x = f/self.peakFrequency_primarysw
        return x**3 * (7 / (4 + 3*x**2))**3.5
        
    def spectralShapeturb(self, f: float) -> float:         #spectralShape 1 turblence
    ...
    def spectralShape_doubleBrokensw(self, f: float):     #spectralShape 2 turblence
    ...
    def getGWfuncturb(self, doubleBroken: bool = True) -> Callable[[float], float]:
        if doubleBroken:
            return lambda f: self.peakAmplitudeturb*self.spectralShape_doubleBrokenturb(f)    
        else:
...
class GWAnalyser:
    ...
    def __init__(self, detectorClass: Type[GWDetector], potentialClass: Type[AnalysablePotential], outputFolder: str):
        ...
        for transitionReport in self.relevantTransitions:
            ...
            gwFunc_singleturb = gws.getGWfuncturb(doubleBroken=False)
            gwFunc_doubleturb = gws.getGWfuncturb(doubleBroken=True)
